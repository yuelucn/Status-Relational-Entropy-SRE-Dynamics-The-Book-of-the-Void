"""
DEPRECATED — v1 reference implementation. Superseded by `sre_core.py` (v2).

Known defects, all fixed in sre_core.py:
  1. `M_next[v_m, v_f] = -M_next[v_f, v_m]` forces M to be globally ANTI-symmetric.
     Feeding that to sklearn MDS raises "Array must be symmetric". v2 keeps M
     generally non-symmetric and reconstructs geometry from S = (M + M^T)/2 only.
  2. Lambda is absent: the pruning gain is taken solely from the endogenous
     spectral-radius term lambda(n), so the "Lambda controls the transition"
     narrative has no handle here. v2 reintroduces Lambda (modes A and B).
  3. `current_phi = active_channels / n` is identically 1 whenever pruning is
     inactive, so Phi == 1.0000 is a construction artefact, not evidence.

Kept for provenance only. Do not cite numbers produced by this file.
See n-s.md v2 section 0 for the full revision list.
"""
import numpy as np
